    ...
    <div class="card card-primary card-outline">
        ...
        <div class="card-header">
            <div class="card-tools">
                @can('pos_create')
                <a class="btn btn-primary btn-sm" href="{{ route("admin.pos.create") }}">
                    <i class="mr-1 fas fa-plus"></i>{{ trans('global.add') }} {{ trans('global.pos.title_singular') }}
                </a>
                @endcan
            </div>
        </div>
                    ...
                    <div class="col-sm-12">
                        <div class="box-body">
                            <div class="table-responsive-lg data-table-wrapper">
                                <table id="pos_table" class="table  table-hover table-bordered datatable cell-border">
                                    <thead>
                                        <tr>
...
<script>
    $(function() {
        ...
        // Ajax Data Load
        var dataTable = $('#pos_table').DataTable({
            processing: true,
            serverSide: true,
            ajax: {
                url: '{{ route("admin.pos.get") }}',
                headers: {
        ...
        $(document).on('click', '.change_status', function(e) {
            ...
            $.ajax({
                url: "{{URL('admin/pos/changeStatus')}}",
                type: "POST",
                cache: false,
                data: {
                    _token: '{{ csrf_token() }}',
